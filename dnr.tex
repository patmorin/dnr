...
Let $W$ be a subset of the vertices in a graph $G$.  A separation $(A,B)$ of $G$ is \defin{$W$-balanced} if $|(A\setminus B)\cap W|\le \tfrac{2}{3}|W|$ and $|(B\setminus A)\cap W|\le \tfrac{2}{3}|W|$.
...
% \hussein{Hi Pat, did you read the comment above, so I can delete it?}
% \pat{Yes, let's just keep the $s_i$ notation.}
...
Observe that, for any $W$-sequence $W_0,\ldots,W_{\ell+1}$, we have the bounds $(\ell+1)|W|\le |W_{\ell+1}|< (\ell+2)|W|$, so $1/(\ell+2)< |W|/|W_{\ell+1}|\le 1/(\ell+1)$.  
The following lemma shows that, for any separation $(A,B)$ of $G[W_{\ell+1}]$, the size of the intersection of $A\setminus B$ with $W\cup Z$ is bounded by the order $|A\cap B|$ of $(A,B)$ and the ratio $|A\setminus B|\cdot |W|/|W_{\ell+1}|$.
